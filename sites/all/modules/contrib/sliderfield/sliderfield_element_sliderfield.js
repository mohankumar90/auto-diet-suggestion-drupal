  ...
  var sliderfieldsSlideUpdateFields = function($slider, ui) {
    ...
    for(var i = 0; i < $values.length; i++) {
      ...
      $values[i] = setting.display_values_format.replace('%{value}%', $values[i]);
        ...
        /**************  Custom code starts here for CHO, PROTEIN and FAT values calculate ************
    $energy = parseFloat($('#energy').html());

    $val = 0;

    if ($slider.find('#edit-fat-slider-container').length == 0)
      $val = parseInt($values/400 * $energy);
    else
      $val = parseInt($values/900 * $energy);

    $slider.find('.sliderfield-display-values-field').html($val + " g/day"); //(($values/100 * $energy)/9) + '' );
